TestCase("[Table] GetScore / GetMaxScore / GetErrorCount", {
    ...
    'test select gap - given answer containing an alt text, when onEdit is called, set correct value': function () {
        let gap1 = new this.presenter.SelectGap("gapId", ["\\alt{hello|world}","wrong answer"], 1);
        gap1.$view = {
            find: (query)=>{
                if (query === ':selected') {
                    return {
                        val: ()=>{return "\\alt{hello|world}";},
                        text: () => {return "hello";}
                    }
                } else {
                    return {
                        val: ()=>{return "wrong answer";},
                        text: ()=>{return "wrong answer";}
                    }
                }
            }
        }
        let notifyEditStub = sinon.stub(gap1, "notifyEdit");
        let notifyStub = sinon.stub(gap1, "notify");

        gap1.onEdit();

        assertEquals("\\alt{hello|world}", gap1.value);
    }
});