function AddonQuiz_create() {
    ...
    */
    ...
    function simpleChangePolishWordGender(word, gender) {
    ...
    function partOfNumberToBigElementOfPolishNumber(n, wordsDescribingNumber, skipZero=false) {
        if (n === 0 && skipZero)
            return '';
    ...
    function numberToPolishNumber(n) {
        if (n === 0)
            return 'zero';
        ...
        var nAsText = '';
        nAsText += partOfNumberToBigElementOfPolishNumber(billions, ['miliard', 'miliardy', 'miliard\u00f3w'], true) + ' ';
        nAsText += partOfNumberToBigElementOfPolishNumber(millions, ['milion', 'miliony', 'milion\u00f3w'], true) + ' ';
        nAsText += partOfNumberToBigElementOfPolishNumber(thousands, ['tysi\u0105c', 'tysi\u0105ce', 'tysi\u0119cy'], true) + ' ';
        ...
        return removeExtraSpaces(nAsText);
    }

    function getOrdinalPartOfNumber(n) {
        ...
        ordinalPart = reverseString(reversedOrdinalPart);
        return parseInt(ordinalPart);
    }

    function getTextNonOrdinalPartOfNumber(n) {
        numberLength = n.toString().length;
        var ordinalPartAsStr = getOrdinalPartOfNumber(n).toString();
        ...
        if (ordinalPartIsABigNumber && nonOrdinalPartContainsOrdinalPartSize)
            nAsText = removeLastWord(nAsText);

        return removeExtraSpaces(nAsText)
    }

    function getTextOrdinalPartOfNumber(n, gender=GENDER.MASCULINE) {
        ...
        [textUnits, textTeens, textTens, textHundreds, sizes].forEach((_, i, arr) => {
            arr[i].forEach((str, j, arr) => arr[j] = simpleChangePolishWordGender(str, gender));
        });
        if (gender === GENDER.FEMININE) {
            textUnits[2] = 'druga';
        }

        var ordinalPart = getOrdinalPartOfNumber(n);
        ...
        return removeExtraSpaces(ordinalPartAsText);
    }

    function numberToPolishOrdinalNumber(n, gender=GENDER.MASCULINE) {
        if (n === 0)
            return simpleChangePolishWordGender('zerowy', gender);
        return getTextNonOrdinalPartOfNumber(n) + ' ' + getTextOrdinalPartOfNumber(n, gender);
    }

    function numberToOrdinalNumber(n, language, gender=GENDER.MASCULINE) {
        if (['pl', 'polish'].includes(language.toLowerCase()))
    ...
    function getQuestionTitleTextVoiceObject($element) {
        ...
        var language = presenter.config.langTag;
        if (language === '')
            language = document.documentElement.lang;

        const currentQuestionAsText = numberToOrdinalNumber(state.currentQuestion, language, GENDER.NEUTER);
        const questionsLengthAsText = numberToOrdinalNumber(presenter.config.questions.length, language, GENDER.FEMININE);
