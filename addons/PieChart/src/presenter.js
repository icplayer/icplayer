function AddonPieChart_create(){
	...
	presenter.drawGraph = function() {
		...
		var $svg = '<svg height="'+graphSize+'" width="'+graphSize+'" version="1.1" xmlns:xlink="http://www.w3.org/2000/svg">';
		angle = 0;
		lines = '';
		percents = '';
		...
		for (i = 0; i < presenter.numberOfItems; i++) {
			...
			angle2 = angle + 0.5*parseFloat(presenter.items[i]['Starting percent'])/50 * Math.PI;
			angle += parseFloat(presenter.items[i]['Starting percent'])/50 * Math.PI;
			...
			(parseFloat(presenter.items[i]['Starting percent']) > 50) ? (ItemData += '1') : (ItemData += '0');
			ItemData += ' 1 '+x2+' '+y2+' L '+presenter.center+' '+presenter.center+' Z';
			presenter.startingItems[i] = ItemData;
			...
			if (presenter.values) {
				percents += presenter.drawPercent(i,x3,y3,presenter.currentPercents[i]);
			}
			presenter.angles[i] = ((angle/Math.PI*180-180)+360)%360;
			presenter.startingLines[i] = presenter.angles[i];
			lines += '<rect id="'+(i+1)+'" class ="line" height="'+presenter.radius+'" width="2" y="'+presenter.center+'" x="'+(presenter.center-1)+'" transform="rotate('+presenter.angles[i] +', '+presenter.center+', '+presenter.center+')"></rect>';			
		}
		$svg += '<circle class="graph" r="'+presenter.radius+'" cy="'+presenter.center+'" cx="'+presenter.center+'"></circle>';
		$svg += lines;
		$svg += percents;
		$svg += '</svg>';
		presenter.piechart.append($svg);
	};
	...
	presenter.run = function(view, model){	
		...
		presenter.$view
			.click(function(e) {
				e.preventDefault();
				e.stopPropagation();
				if (presenter.isLineInMove && !presenter.isErrorCheckingMode && !presenter.disabled) {
			...
			.mousemove(function(e) {
				e.preventDefault();
				e.stopPropagation();	
				if (presenter.isLineInMove 	&& !presenter.disabled && !presenter.isErrorCheckingMode) {
		...
		presenter.$view.find('.piechart').on('touchmove', function(e){
			...
			if (presenter.isLineInMove && (x < -presenter.center || y < -presenter.center || x >presenter.center || y > presenter.center && !presenter.disabled && !presenter.isErrorCheckingMode)) {
				presenter.isLineInMove = false;
				(presenter.isAllOK()) ? (score=1) : (score=0);
				if (!presenter.activity) score ='';
				presenter.triggerLineEvent(score);
