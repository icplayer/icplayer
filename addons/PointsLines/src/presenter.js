function AddonPointsLines_create(){
	...
	function getLines(dataLines, numberOfPoints,stars) {
		...
		for (i = 0; i<addLine.length; i++) {
			...
			} else if (stars && !isNaN(tmp_dane[0]) && tmp_dane[0] <= numberOfPoints && !isNaN(tmp_dane[1].substring(0,tmp_dane[1].length-1)) && (tmp_dane[1].substring(tmp_dane[1].length-1)) === '*' && tmp_dane[1].substring(0,tmp_dane[1].length-1) <= numberOfPoints) {
				if (tmp_dane[0] > tmp_dane[1].substring(0,tmp_dane[1].length-1)) {
					point1 = tmp_dane[1].substring(0,tmp_dane[1].length-1)-1;
					point2 = tmp_dane[0]-1;
				} else {
					point1 = tmp_dane[0]-1;
					point2 = tmp_dane[1].substring(0,tmp_dane[1].length-1)-1;
				}
