...
public class TextPresenter implements IPresenter, IStateful, IActivity, ICommandReceiver {
	...
	private void showAnswers() {
		com.lorepo.icf.utils.JavaScriptUtils.log(module.getId() + " [showAnswers]");
	...
	private void hideAnswers() {
		com.lorepo.icf.utils.JavaScriptUtils.log(module.getId() + " [hideAnswers]");
		if (!module.isActivity()) {
			for (int i = 0; i < view.getChildrenCount(); i++) {
				TextElementDisplay child = view.getChild(i);
	...
	public void setWorkMode() {
		com.lorepo.icf.utils.JavaScriptUtils.log(module.getId() + " [setWorkMode]");
		if (isShowAnswers()) {
			hideAnswers();
		}
	...
	public void setShowErrorsMode() {
		com.lorepo.icf.utils.JavaScriptUtils.log(module.getId() + " [setShowErrorsMode]");
		if (isShowAnswers()) {
			hideAnswers();
		}
	...
	private void setGapAnswer(int index, String answer) {
		com.lorepo.icf.utils.JavaScriptUtils.log(module.getId() + " [setGapAnswer]");
		if (view != null && index <= view.getChildrenCount()) {
