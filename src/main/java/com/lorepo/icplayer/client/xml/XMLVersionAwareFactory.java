...
public abstract class XMLVersionAwareFactory implements IXMLFactory {
	...
	public void load(String fetchUrl, IProducingLoadingListener listener) {
		try {
			RequestsUtils.get(fetchUrl, this.getContentLoadCallback(listener, fetchUrl));
		...
		return new RequestCallback() {
			...
			public void onResponseReceived(Request request, Response response) {
				if (response.getStatusCode() == 200 || response.getStatusCode() == 0) {
					Object producedItem = produce(response.getText(), url);
	...
	protected Object produce(String xmlString, String fetchUrl) {
		...
		JavaScriptUtils.log("wersja xmla page");
		JavaScriptUtils.log(version);
		JavaScriptUtils.log("pobieram parsera");
		...
		JavaScriptUtils.log(producedContent);
